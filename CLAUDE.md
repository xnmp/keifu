...
- **Interactive-rebase durable state has two ownership phases.** Before Git creates `.git/rebase-merge/interactive`, the starter's `.git/keifu-interactive-rebase.lock` prevents another app from deleting state during hooks; afterward the Git marker is authoritative and a second start must refuse before writing shared files. The todo/message directory also carries the pre-HEAD/count/base undo seed across pause/relaunch; successful Continue records it before cleanup. Failed starts, completion, external abort, startup, and refresh reconcile unlocked orphans.
