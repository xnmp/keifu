...
- **Network jobs stay on the Git CLI and have one lifecycle source of truth.** Fetch/pull/push use forced Git progress plus HTTP low-speed bounds; advancing bytes/objects/refs resets a 60-second inactivity window. `NetworkManager` owns running/cancelling/integrating/busy state, and the status bar renders it directly. On Unix, cancel the owned Git process tree with bounded SIGINT → SIGTERM → SIGKILL escalation; retain discovered process-group members/descendants as PID/birth-identity pairs across direct-child exit, revalidate before every signal, and keep helper discovery, direct-child reaping, and post-exit pipe draining deadline-bound. Non-Unix builds do not advertise/accept cancellation until they have an integrity-safe group primitive. Pull integration uses an acknowledged lifecycle barrier, gates repository mutations, preserves cancellation that wins the acknowledgement race, revalidates HEAD, and then finishes non-cancellably so index/worktree mutation cannot be interrupted. Keep the job busy until the transport tree and any local integration exit, then emit one outcome toast and release the slot. Normal and forced quit defer `should_quit` while Unix transport cancellation/reaping is bounded or pull integration is active on any platform; a non-Unix running transport exits immediately because keifu cannot safely terminate its process tree.
