...
- **Interactive-rebase durable state has two ownership phases.** Before Git creates `.git/rebase-merge/interactive`, the starter's `.git/keifu-interactive-rebase.lock` prevents another app from deleting state during hooks; afterward the Git marker is authoritative and a second start must refuse before writing shared files. Continue/Abort reacquire that lock before Git and retain it through undo-seed consumption and cleanup, including the instant after Git removes its marker. The todo/message directory carries the pre-HEAD/count/base seed across pause/relaunch; failed starts, completion, external abort, startup, and refresh reconcile only unlocked orphans.
