...
- **Interactive-rebase durable state has two ownership phases.** Before Git creates `.git/rebase-merge/interactive`, the starter's `.git/keifu-interactive-rebase.lock` prevents another app from deleting state during hooks; afterward the Git marker is authoritative. Failed starts, completion, external abort, startup, and refresh must reconcile unlocked orphaned todo/message files.
