...
- Tests that temporarily change process-global Git environment variables belong
  in a dedicated integration-test binary, never alongside parallel siblings.
...
- **Keyboard-shortcut editing is transactional.** The Ctrl+, → Ctrl+K editor keeps a pending TOML table until Ctrl+S; capture bypasses normal shortcut routing, and Esc either cancels capture or discards the whole pending edit. Keep the fixed editor navigation independent of configurable bindings so users can always repair them.
