...
- **Network jobs stay on the Git CLI and have one lifecycle source of truth.** Fetch/pull/push use forced Git progress plus HTTP low-speed bounds; advancing bytes/objects/refs resets a 60-second inactivity window. `NetworkManager` owns running/cancelling/busy state, the status bar renders it directly, and `x` requests cooperative cancellation. On Unix, cancel the Git process group with bounded SIGINT → SIGTERM → SIGKILL escalation. Non-Unix builds do not advertise or accept cancellation until they have an integrity-safe group primitive; never fall back to force-killing only Git's direct child. Pull integration is a separate non-cancellable local step so cancellation cannot interrupt index/worktree mutation. Keep the job busy until the transport group and any local integration exit, then emit one outcome toast and release the slot.
