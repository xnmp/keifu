...
- **Git's interactive marker owns durable rebase state.** Keep `.git/keifu-interactive-rebase` only while `.git/rebase-merge/interactive` exists; failed starts, completion, external abort, startup, and refresh must reconcile orphaned todo/message files.
