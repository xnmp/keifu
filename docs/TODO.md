...
### [DONE] #102 graph_view.rs decomposition (tracked as GH issues #75-#81)
...
Landed as a package split: mod.rs 4899 → ~1950 lines plus metrics/geometry/rows/chips/badges/pixel_dim/row modules; typed seams (BranchChip carries its resolved branch, PrBadge, pure resolve_row_model → RowModel + layout_row). Rendering verified byte-identical by adversarial diff review; decision tests assert on RowModel/BranchChip/PrBadge fields, layout tests keep full renders. Contract-pinning tests surfaced two pre-existing gaps: an abbreviated (ellipsized) branch chip resolves to no branch so clicks do nothing, and merged-lane muting is the only category without a HEAD exemption.
...
### [DONE] #119 Pixel graph: orphan vertical stub where the uncommitted band lands under a folded connector
The `/|` artifact, round 4 (reported on a detached HEAD + uncommitted files: the tip row is the uncommitted band, whose landing corner sits directly under a folded connector row). Two halves of one stub at the band's landing column: (1) the folded connector's Pipe shadow rasterized a straight half in the host row even though the host's own cell there is the terminating `MergeLeft` corner — the transition cubic already spans from the row above's mid-height into the run, so the whole shadow is superseded; (2) the row above's own pipe drew its bottom half to the row edge because its merged below-view showed the connector's Pipe instead of the corner it shadows — the curve-fed flags now look through to the neighbour row's raw cells. Also: `draw_cells` skips a fully curve-fed vertical instead of drawing a degenerate zero-length segment (which painted a stroke-radius blob). Pixel renderer only; text mode keeps the connector as its own row and was always correct. Regression test `band_landing_under_folded_connector_leaves_no_stub` (fails pre-fix), plus offline before/after rasters via the new `render_rows` example.
