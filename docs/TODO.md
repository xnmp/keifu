...
### [DONE] #119 Hide-remote-branches keeps upstreams of local branches visible (GH #105)
`remote_only_branch_names` hid every remote whose tip didn't exactly match a local branch (#57's fix), which over-hid: a local branch's own configured upstream (e.g. `origin/feature` for local `feature`) was hidden whenever the two had diverged, masking pushed-but-unpulled updates. Now a remote also stays visible when some local branch's `upstream` names it, alongside the existing exact-tip-match rule; #57's short-name/no-upstream case is unaffected (`remote_sharing_short_name_but_different_tip_is_remote_only`).
