...
The plan lives on `App`, while `AppMode::RebasePlan` carries only its cursor. This preserves the plan when reword input and confirmation temporarily replace the mode. It also records the exact branch ref and HEAD displayed to the user. Under the execution lock, confirmation reopens the repository and refuses if that ref, HEAD, raw Git operation state, or worktree cleanliness changed before it materializes a durable todo, reword-message files, and the pre-rebase guarded-undo seed under `.git/keifu-interactive-rebase`. `GIT_SEQUENCE_EDITOR` hands that todo to `git rebase -i --empty=drop`: Git owns rewriting, signing, hooks, conflicts, and crash recovery, while a commit made empty by the plan is dropped instead of entering a pause the UI cannot resolve. Keifu consumes the seed to record a guarded hard-reset undo only after completion, including completion through Continue after relaunch. State files remain while a CLI rebase is paused and are removed on completion or abort.
