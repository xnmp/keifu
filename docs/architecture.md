...
The plan lives on `App`, while `AppMode::RebasePlan` carries only its cursor. This preserves the plan when reword input and confirmation temporarily replace the mode. It also records the exact branch ref and HEAD displayed to the user. Under the execution lock, confirmation reopens the repository and refuses if that ref, HEAD, raw Git operation state, or worktree cleanliness changed before it materializes a durable todo, reword-message files, and the pre-rebase guarded-undo seed under `.git/keifu-interactive-rebase`. `GIT_SEQUENCE_EDITOR` hands that todo to `git rebase -i --empty=drop --reschedule-failed-exec`: Git owns rewriting, signing, hooks, conflicts, and crash recovery, while a commit made empty by the plan is dropped instead of entering a pause the UI cannot resolve. Reword is encoded as `exec git commit --amend`; rescheduling ensures Continue retries that exact authored command after a transient hook/exec failure instead of silently completing with the old message. Keifu consumes the seed to record a guarded hard-reset undo only after completion, including completion through Continue after relaunch. State files remain while a CLI rebase is paused and are removed on completion or abort.

### Interactive-rebase crash and concurrency lifecycle

The Git marker `.git/rebase-merge/interactive` is authoritative once Git starts. Before that marker exists, the starter holds an exclusive `.git/keifu-interactive-rebase.lock` while materializing state and waiting for Git (including hooks). Startup and refresh only reconcile state after acquiring that lock: a live starter therefore keeps its files, while a failed start cleans immediately and a later startup removes crash/completion/external-abort orphans.

| Crash or external event | Observable recovery | Behavioural coverage |
| --- | --- | --- |
| Another process commits or checks out a branch while the plan/review overlay is open | Confirmation compares a freshly opened repository with the displayed branch ref and HEAD while holding the state lock. It reports a non-blocking error and leaves the externally changed history untouched before any Keifu state is written. The same boundary rechecks raw Git operation state and worktree cleanliness. | `confirmed_plan_refuses_an_external_commit_after_review`, `confirmed_plan_refuses_a_branch_switch_after_review` |
| After Keifu creates state, before Git creates its marker (including a long-running or rejecting `pre-rebase` hook) | The live starter's lock prevents concurrent startup/refresh from deleting its files. A failed call cleans before releasing ownership; after a crash releases the lock, relaunch removes the orphan. | `concurrent_startup_does_not_delete_state_while_a_rebase_is_starting`, `failed_confirmed_plan_is_a_non_blocking_error_toast`, `startup_removes_orphaned_interactive_rebase_state` |
...
| During a conflict or other pause | Relaunch/refresh detects the CLI rebase; Continue or Abort uses Git CLI and preserves both reword content and the guarded-undo seed. A successful Continue records the undo before cleanup. | `reword_message_file_survives_a_conflict_until_continue_reaches_it`, `confirmed_conflicting_plan_enters_the_existing_conflict_workflow`, `continued_interactive_rebase_after_restart_can_be_undone` |
| After Git completes, before Keifu removes state | Continue retains the Keifu state lock while Git removes its marker, then consumes the undo seed and cleans up; startup and new-plan attempts cannot delete or replace files during that handoff. If the process dies first, the released lock lets the next startup remove the orphan. | `confirmed_plan_refreshes_history_and_toasts_success`, `continued_interactive_rebase_after_restart_can_be_undone`, `completion_handoff_blocks_a_new_plan_until_undo_and_cleanup_finish`, `startup_removes_orphaned_interactive_rebase_state` |
...
Concurrency has two phases. Keifu's advisory lock serializes both reviewed-snapshot validation and state ownership before the Git marker exists; a second Keifu process leaves locked files untouched and a second start reports a non-blocking error. Once Git starts, the marker is checked again under the Keifu lock before any shared file is materialized: a second start is refused without changing the active todo, pending message, or undo seed (`second_instance_cannot_overwrite_pending_reword_of_paused_rebase`). Continue and Abort reacquire that same lock before invoking Git and retain it through undo consumption/cleanup, closing the marker-removal handoff. Git's single per-repository rebase state then decides recovery. Keifu never treats its own directory alone as proof that a rebase is active.
...
1. `PixelGraphState::new()` (`ui/graph_pixels.rs`) calls
   ...
   *transparency-preserving* protocol is detected: only **Kitty** and **iTerm2**
   ...
   On each crossterm resize event, `PixelGraphState` derives the latest cell
