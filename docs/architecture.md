...
Concurrency has two phases. Keifu's advisory lock serializes and owns state before the Git marker exists; a second Keifu process leaves locked files untouched and a second start reports a non-blocking error. Once Git starts, Git's single per-repository rebase state serializes history operations and its marker decides recovery. Keifu never treats its own directory alone as proof that a rebase is active.
