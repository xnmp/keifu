...
| After Git completes, before Keifu removes state | Completion consumes the undo seed before cleanup; if the process dies first, the missing Git marker makes the next startup or refresh remove the orphan. | `confirmed_plan_refreshes_history_and_toasts_success`, `continued_interactive_rebase_after_restart_can_be_undone`, `startup_removes_orphaned_interactive_rebase_state` |
